...
from app_real_estate.models import AssociateRatings
...
async def read_rating_by_id_db(session: AsyncSession, rating_id: int) -> RatingSchema | None:
    return await session.get(AssociateRatings, rating_id)


async def create_rating_db(session: AsyncSession, rating_in: RatingSchema) -> AssociateRatings:
